...
/**
 * @author bing  @create 2020/5/17 3:48 下午
 */
...
public class ElasticsearchTest {
    ...
    @Autowired
    private DiscussPostMapper discussPostMapper;//取数据

    @Autowired
    private DiscussPostRepository discussPostRepository;//存数据

    @Autowired
    private ElasticsearchTemplate elasticTemplate;//解决解决不了情况
    ...
    public void testUpdate(){
        ...
        // GET = localhost:9200/discusspost/_doc/231
    }

    @Test // 删除数据
    public void testDelete(){
//        discussPostRepository.deleteById(231);// 删除1条数据 GET = localhost:9200/discusspost/_doc/231
        discussPostRepository.deleteAll();//删除所有数据 GET = localhost:9200/discusspost/_search
    }
    ...
    public void testSearchByRepository(){
        SearchQuery searchQuery = new NativeSearchQueryBuilder()
                ...
                .withHighlightFields(
                        new HighlightBuilder.Field("title").preTags("<em>").postTags("</em>"), //标签高亮
                        new HighlightBuilder.Field("content").preTags("<em>").postTags("</em>")
                ).build();

        //elasticTemplate.queryForPage(searchQuery,class,SearchResultMapper)
        // 底层获取到了高亮显示的值，但是没有返回。（缺陷）
        Page<DiscussPost> page = discussPostRepository.search(searchQuery);
        System.out.println(page.getTotalElements());//共有多少条数据匹配
        System.out.println(page.getTotalPages());//按当前分页条件，共多少页
        System.out.println(page.getNumber());//当前处在第几页
        System.out.println(page.getSize());//每页显示多少条数据
        for (DiscussPost post : page) {
            System.out.println(post);
        }
    }

    @Test // 使用 elasticTemplate 搜索；带高亮标签
